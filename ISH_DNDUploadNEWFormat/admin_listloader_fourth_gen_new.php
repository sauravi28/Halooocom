...
if ( (!$OK_to_process) or ( ($leadfile) and ($file_layout!="standard" && $file_layout!="template") ) )
	...
	if ($file_layout!="custom") 
		{
		?>
		<table align=center width="1150" border=0 cellpadding=9 cellspacing=0 bgcolor=#a5d26b>
		  <tr>
		...
		if (count($field_check)>=2) 
			...
			while (!feof($file)) 
				...
				if (strlen($buffer)>0) 
					...
					$status =				"NEW";
					$user ="";
					$vendor_lead_code =		$row[9];
					$source_code =			"";
					$source_id=$source_code;
					$list_id =				"";
					$gmt_offset =			'0';
					$called_since_last_reset='N';
					$phone_code =			preg_replace('/[^0-9]/i', '', "");
					$phone_number =			preg_replace('/[^0-9]/i', '', $row[10]);
					$title =				"";
					$first_name =			$row[0];
					$middle_initial =		$row[1];											$excel_date = $row[2]; //here is that excel value 41621 or 41631					//Convert excel date to mysql db date						  $unix_date = ($excel_date - 25569) * 86400;						  $excel_date = 25569 + ($unix_date / 86400);						  $unix_date = ($excel_date - 25569) * 86400;						 //Insert below to sql					    $last_name = gmdate("Y-m-d", $unix_date);															//$last_name =			$row[2];						$excel_date1 = $row[3];						  $unix_date1 = ($excel_date1 - 25569) * 86400;						  $excel_date1 = 25569 + ($unix_date1 / 86400);						  $unix_date1 = ($excel_date1 - 25569) * 86400;						 //Insert below to sql					    $address1 = gmdate("Y-m-d", $unix_date1);									
					//$address1 = $row[3];
					$address2 =				$row[4];
					$address3 =				$row[5];
					$city =					$row[6];
					...
					$country_code =			"";
					$gender =				"";
					$date_of_birth =		"";										$excel_date11 = $row[11];						  $unix_date11 = ($excel_date11 - 25569) * 86400;						  $excel_date11 = 25569 + ($unix_date11 / 86400);						  $unix_date11 = ($excel_date11 - 25569) * 86400;						 //Insert below to sql					    $alt_phone = gmdate("Y-m-d", $unix_date11);									
					//$alt_phone =			preg_replace('/[^0-9]/i', '', $row[11]);
					$email =				$row[13];										$excel_date2 = $row[12];						  $unix_date2 = ($excel_date2 - 25569) * 86400;						  $excel_date2 = 25569 + ($unix_date2 / 86400);						  $unix_date2 = ($excel_date2 - 25569) * 86400;						 //Insert below to sql					    $security_phrase = gmdate("Y-m-d", $unix_date2);
					//$security_phrase =		$row[12];
					$comments =				trim($row[22]);										$excel_date3 = $row[14];						  $unix_date3 = ($excel_date3 - 25569) * 86400;						  $excel_date3 = 25569 + ($unix_date3 / 86400);						  $unix_date3 = ($excel_date3 - 25569) * 86400;						 //Insert below to sql					    $field1 = gmdate("Y-m-d", $unix_date3);										//$field1 =				$row[14];					$field2 =				$row[15];					$field3 =				$row[16];					$field4 =				$row[17];					$field5 =				$row[18];										$field6 =				$row[19];					$field7 =				$row[20];					$field8 =				$row[21];					$field9 =				$row[22];					$field10 =				$row[23];										$field11 =				$row[24];					$field12 =				$row[25];					$field13 =				$row[26];					$field14 =				$row[27];					$field15 =				$row[28];					$field16 =				$row[29];					$field17 =				$row[30];										$field18 =				"";					$field19 =				$row[31];					$field20 =				"";										$field21 =				$row[32];					$field22 =				$row[33];					$field23 =				$row[34];					$field24 =				$row[35];					$field25 =				$row[36];										$field26 =				"";					$field27 =				"";					$field28 =				"";					$field29 =				"";					$field30 =				"";										$field31 =				"";					$field32 =				"";					$field33 =				"";					$field34 =				"";					$field35 =				"";					$field36 =				"";					$field37 =				"";					$field38 =				"";					$field39 =				"";					$field40 =				"";										
					$rank =					"";
					$owner =				"";
