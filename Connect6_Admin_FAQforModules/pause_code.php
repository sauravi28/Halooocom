...
if($loggedInuserName == '' && $loggedInPassword == '' ){
	header('Location:/haloocomConnect5/pages/Agent/connect5Login/index.php');
		...
		if($_POST['addPhone']){
		   ...
		   $pause_name = $_POST['pause_name'];
			
			$stmt_rs="SELECT pause_id from pause_code where pause_id='$pause_id';";
					...
					$stmt_insert="INSERT INTO pause_code(created_date,pause_id,pause_name) values('$date','$pause_id','$pause_name')";
					$rslt_insert= mysqli_query($conn,$stmt_insert);
					       
							}else{
								echo "<script>alert('Pause Code id already exist.');</script>";
							}
	}
	
	if($_POST['editPhone'] == 'edit_Phone'){  
			$txt_pause_id = $_POST['txt_pause_id'];
			$txt_pause_name = $_POST['txt_pause_name'];
			$txt_id = $_POST['txt_id'];
			
		 $stmt_update = "update pause_code set pause_id='$txt_pause_id',pause_name='$txt_pause_name' where id='$txt_id'";
			$rslt_update= mysqli_query($conn,$stmt_update);		
	}
	
	if($_POST['deletePhone'] == 'delete_Phone'){	
			$deletePhoneID = $_POST['deletePhoneID'];
			
		 $stmt_delete = "delete from pause_code where id='$deletePhoneID'";
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Pause Codes</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
      <div class="container-fluid">
		  ...
		  <?php if($loggedInUserLevel =="SuperAdmin"){?>
			<button type="submit" class="btn btn-primary" data-toggle="modal" data-target="#myModal">ADD PAUSE CODES</button>
			<?php } ?>
		   </div>
        ...
        <div class="card card-default">
          <div class="card-header">
            <h3 class="card-title">PAUSE CODES LISTINGS</h3>
			 ...
			 $stmt_select="SELECT * from pause_code";
			  ...
			  ?>
                  <tr>
                      ...
                      <td>
                          <a>
                           <?php echo $row["pause_id"]; ?>
                          </a>
                      </td>
					  <td>
                          <?php echo $row["pause_name"]; ?>
                      </td>
                   
					  <?php if($loggedInUserLevel =="SuperAdmin"){?>
                      <td class="project-actions text-right">
                         <!-- <a class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal_edit"> -->
						 <span class="btn btn-info btn-sm"  onclick = "document.getElementById('myModal_edit').style.display='block'; phoneEditFun('<?php echo $row["id"]; ?>')">
                              <i class="fas fa-pencil-alt">
                              </i>
                              Edit
							  </span>
					   
					    <span class="btn btn-danger btn-sm"  onclick = "document.getElementById('myModal_delete').style.display='block'; phoneDeleteFun('<?php echo $row["id"]; ?>','<?php echo $row["pause_id"]; ?>')">
    ...
    <div class="modal-dialog">
      <div class="modal-content" style="top:40px;">
        ...
        <div class="modal-header">
          <h4 class="modal-title">EDIT PAUSE CODE</h4>
             ...
             <form action = "<?php echo $_SERVER['PHP_SELF']; ?>" method = "POST">
                <div class="card-body">
                  <div class="form-group">
                    <label for="CampaignID">Pause Code ID<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" id="txt_pause_id" name="txt_pause_id" pattern="^[a-zA-Z0-9\s]*$" placeholder="Enter Pause Code ID" readonly>
                  </div>
				  <div class="form-group">
                    <label for="PhoneID">Pause Code Name<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" id="txt_pause_name" name="txt_pause_name" pattern="^[a-zA-Z0-9\s]*$"  placeholder="Enter Pause Code Name" required>
                  </div>
                ...
                <div class="card-footer">
				<input type = "hidden" value="edit_Phone" name="editPhone" >
  ...
  <div class="modal fade" id="myModal">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">ADD A NEW PAUSE CODE</h4>
        ...
        <div class="modal-body">
         <form action = "<?php echo $_SERVER['PHP_SELF']; ?>" method = "POST">
                <div class="card-body">
                  <div class="form-group">
                    <label for="PhoneID">Pause Code ID<span style="color:red;">*</span></label> 
                    <input type="text" class="form-control" id="pause_id" name="pause_id" pattern="^[a-zA-Z0-9\s]*$" placeholder="Enter Pause Code ID" required>
                  </div>
				   <div class="form-group">
                    <label for="PhoneID">Pause Code Name<span style="color:red;">*</span></label>  
                    <input type="text" class="form-control" id="pause_name" name="pause_name" pattern="^[a-zA-Z0-9\s]*$"  placeholder="Enter Pause Code Name" required>
                  </div>
                ...
                <div class="card-footer"> 
                  <input  type="submit" class="btn btn-primary" name ="addPhone" value = "Submit">
                </div>
              </form>
        </div>
  ...
  <div class="modal fade" id="myModal_delete" style="opacity: 3;top : 104px !important">
    <div class="modal-dialog">
      <div class="modal-content" style="top:40px;">
        ...
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">DELETE PAUSE CODE</h4>
        ...
        <div class="modal-body">
         <form action = "" method = "POST">
                <div class="card-body">
                  <div class="form-group">
						<label for="UserID">Pause Code ID</label>
						<input type="text" class="form-control" id="txt_deletePhone_id" name="txt_deletePhone_id" readonly>
					</div>
				
                </div>
				<input type = "hidden" value = "" id = "deletePhoneID" name = "deletePhoneID">
                <!-- /.card-body -->

                <div class="card-footer">
				  <input type = "hidden" value="delete_Phone" name="deletePhone" >
...
function myFunctionForFAQ() {
  var myWindow = window.open("help/breakFAQ.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=50,width=400,height=400");
}
   ...
   function phoneEditFun(val)
	{
		...
		//alert(res);
			document.getElementById('txt_pause_id').value=res[0];
			document.getElementById('txt_pause_name').value=res[1];
			document.getElementById('txt_id').value=res[2];
		    }
        }; 
		xhttp.open("GET", "/haloocomConnect5/pages/Agent_mongodb/ajax/get_pausecodeedit_data.php?id="+val, true);
		xhttp.send();	  
		// Get the modal
	...
	function phoneDeleteFun(val_grpd,PhoneIDVal)
	{
		document.getElementById('deletePhoneID').value=val_grpd;
		document.getElementById('txt_deletePhone_id').value=PhoneIDVal;
