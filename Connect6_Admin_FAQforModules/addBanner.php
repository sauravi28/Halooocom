...
if($_POST['addlist']){
				...
				$active_announce = $_POST['active_announce'];
						
								
					$date = date("Y-m-d H:i:s");
			$stmt_insert="INSERT INTO addbanner(created_date,user,campaign,announcement,status) values('$date','$users','$campaign','$announcement','$active_announce')";
	...
	if($_POST['editVoiceblast'] == 'edit_Voiceblast'){
			...
			$txt_campaign =$_POST['campaign_txt'];	
            $txt_users ="";				
            $txt_announcement = $_POST['announcement_txt'];
		    $txt_active_announce = $_POST['active_announce_txt'];
			$txt_id = $_POST['txt_id'];
			
			
		 $stmt_update = "update addbanner set user='$txt_users',campaign='$txt_campaign',announcement='$txt_announcement',status='$txt_active_announce' where id='$txt_id'";
	...
	if($_POST['deleteVoiceblasting'] == 'delete_Voiceblast'){	
			...
			$stmt_delete = "delete from addbanner where id='$deleteList'";
 ...
 <!-- Main Sidebar Container -->
  <aside class="main-sidebar sidebar-dark-primary elevation-4" style="background-color: #1A2B6D; color:white;">
    ...
    <a href="../../index.php" class="brand-link" style="background-color: #1A2B6D; color:white;">
      ...
      <span class="brand-text font-weight-light" style="color:#1A2B6D;">Announcement</span>
    </a>
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
		 ...
		 <?php if($loggedInUserLevel =="SuperAdmin"){?>
			<button type="submit" class="btn btn-primary" data-toggle="modal" data-target="#myModal">ADD ANNOUNCEMENT</button>
		 <?php } ?>
		   </div>
        ...
        <div class="card card-default">
          <div class="card-header">
            <h3 class="card-title">ANNOUNCEMENT LISTINGS</h3>
          ...
          <div class="card-body">
            ...
            <div class="row">
             ...
             <table class="table table-striped projects table-bordered" id="table_camp">
              ...
              <tbody>
			   <?php 
							$stmt_select="SELECT user,campaign,announcement,status,id from addbanner";
	                  $rslt_rs= mysqli_query($conn,$stmt_select);
	...
	<span style="background-color:#e60000;border:10px solid #e60000;border-radius:20px;color:white;"><?php echo $row["status"]; ?></span>
					...
					<span class="btn btn-info btn-sm"  onclick = "document.getElementById('myModal_edit').style.display='block'; listuploadedit('<?php echo $row["id"]; ?>')">
                              <i class="fas fa-pencil-alt">
                              </i>
                              Edit
							  </span>
					   
					 <span class="btn btn-danger btn-sm"  onclick = "document.getElementById('myModal_delete').style.display='block'; voiceBlastDeleteFun('<?php echo $row["id"]; ?>','<?php echo $row["announcement"]; ?>')">
         ...
         <form action = "<?php echo $_SERVER['PHP_SELF']; ?>" method = "POST" enctype="multipart/form-data">
                <div class="card-body">
				 <!-- <div class="form-group">
                    <label for="CampaignID">Announcement Assign To</label>
				  ...
				  <div class="form-group" id="campaign_display">
					  ...
					  <select class="form-control select2" style="width: 100%;" id="campaign_txt" name = "campaign_txt" required>
						<option value="">Select</option>
			<?php			
			$stmt_select="SELECT * from campaign";
			   ...
			   while($row = mysqli_fetch_assoc($rslt_rs)) {
					...
					<!--<div class="form-group" id="user_display" style="display:none;">
					  <label>User</label>
					  <select class="form-control select2" style="width: 100%;" id="users_txt" name = "users_txt">
						<option value="">Select</option>
			<?php			
			$stmt_select="SELECT * from users where user_level='D4'";
			   ...
			   while($row = mysqli_fetch_assoc($rslt_rs)) {
                    ...
                    <select class="form-control select2" style="width: 100%;" id="active_announce_txt" name = "active_announce_txt" required>
                        <option value="">Select</option>
                  ...
                  <div class="form-group">
                    <label for="CampaignName">Announcement<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" pattern="^[a-zA-Z0-9\s]*$" title="Please enter only characters and numbers" id="announcement_txt" name="announcement_txt" style="height:50px;" required> 
                  </div>
        ...
        <div class="modal-body">
         <form action ="<?php echo $_SERVER['PHP_SELF'] ?>" method = "POST">
				...
				<!--  <div class="form-group">
				  ...
				  <div class="form-group" id="campaign_display">
					  <label>Campaign<span style="color:red;">*</span></label>
					  <select class="form-control select2" style="width: 100%;" id="campaign" name = "campaign" required>
						<option value="">Select</option>
			<?php			
			$stmt_select="SELECT * from campaign";
	                  $rslt_rs= mysqli_query($conn,$stmt_select);
			...
			$stmt_select="SELECT * from users where user_level='D4'";
			   ...
			   while($row = mysqli_fetch_assoc($rslt_rs)) {
					...
					<div class="form-group">
                    <label for="CampaignName">Active<span style="color:red;">*</span></label>
                    <select class="form-control select2" style="width: 100%;" id="active_announce" name = "active_announce" required>
                        <option value="">Select</option>
                        <option value="Y">Y</option>
                        <option value="N">N</option>
                    </select>
                  ...
                  <div class="form-group">
                    <label for="CampaignName">Announcement<span style="color:red;">*</span></label>
				  <input type="text" class="form-control" pattern="^[a-zA-Z0-9\s]*$" title="Please enter only characters and numbers" id="announcement" name="announcement" style="height:50px;" required> 
                  </div>
      ...
      <div class="modal-content" style="top:40px;">
        ...
        <div class="modal-body">
         <form action = "" method = "POST">
                <div class="card-body">
                  <div class="form-group">
						<label for="UserID">Announcement</label>
...
function myFunctionForFAQ() {
  var myWindow = window.open("help/bannerFAQ.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=50,width=400,height=400");
}
   ...
   function listuploadedit(val)
	{
		var xhttp = new XMLHttpRequest();
		   ...
		   // alert(res);
		    ...
		    document.getElementById('campaign_txt').value=res[0];
		  //  document.getElementById('users_txt').value=res[1];
			document.getElementById('active_announce_txt').value=res[2];
		    document.getElementById('announcement_txt').value=res[3];
		...
		xhttp.open("GET", "/haloocomConnect5/pages/Agent_mongodb/ajax/get_announcement_data.php?id="+val, true);
		xhttp.send();	  
		// Get the modal
