...
if($loggedInuserName == '' && $loggedInPassword == '' ){
	...
	if($_POST['addGroup']){
			...
			$stmt_rs="SELECT group_id from user_group where group_id='$Group_id'";
	                           $rslt_rs= mysqli_query($conn,$stmt_rs);
	                           $row_rs= mysqli_fetch_row($rslt_rs);
							   
							if($row_rs == "" ){			
							
							$stmt_insert="INSERT INTO user_group(group_id,group_name) values('$Group_id','$Group_name')";
							$rslt_insert= mysqli_query($conn,$stmt_insert);
										
								}
								else{
								      echo "<script>alert('Group id already exist.');</script>";
									}
	
							
	}
	
	if($_POST['editGroup'] == 'edit_Group'){  
	 
			$txt_Group_id = $_POST['txt_Group_id'];
			$txt_Group_name = $_POST['txt_Group_name'];
			$txt_id = $_POST['txt_id'];	
			
			$stmt_update = "update user_group set group_id='$txt_Group_id',group_name='$txt_Group_name' where id='$txt_id'";
			$rslt_update= mysqli_query($conn,$stmt_update);	
	
			
	}
	
	if($_POST['deleteGroup'] == 'delete_Group'){	
			$deleteGroupID = $_POST['deleteGroupID'];		
			$stmt_delete = "delete from user_group where id='$deleteGroupID'";
    ...
    <section class="content">
      <div class="container-fluid">
		 ...
		 <?php if($loggedInUserLevel =="SuperAdmin"){?>
			<button type="submit" class="btn btn-primary" data-toggle="modal" data-target="#myModal">ADD USER GROUP</button>
		 <?php } ?>
		   </div>
        ...
        <div class="card card-default">
          <div class="card-header">
            <h3 class="card-title">USER GROUP LISTINGS</h3>
          ...
          <div class="card-body">
            ...
            <div class="row">
             ...
             <table class="table table-striped projects table-bordered" id="table_camp">
              ...
              <tbody>
			   <?php 
						$stmt_select="SELECT * from user_group order by id DESC";
	                  $rslt_rs= mysqli_query($conn,$stmt_select);
                  ...
                  <tr>
                     ...
                     <?php if($loggedInUserLevel =="SuperAdmin"){?>
                      <td class="project-actions text-right">
						 ...
						 <span class="btn btn-info btn-sm"  onclick = "document.getElementById('myModal_edit').style.display='block'; groupEditFun('<?php echo $row["id"]; ?>')">
                              <i class="fas fa-pencil-alt">
                              </i>
                              Edit
							  </span>
					   
					    <span class="btn btn-danger btn-sm"  onclick = "document.getElementById('myModal_delete').style.display='block'; groupDeleteFun('<?php echo $row["id"]; ?>','<?php echo $row["group_id"]; ?>')">
         ...
         <form action = "<?php echo $_SERVER['PHP_SELF']; ?>" method ="POST" enctype="multipart/form-data">
                <div class="card-body">
                  <div class="form-group">
                    <label for="CampaignID">Group ID<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" title="Please enter only characters" id="txt_Group_id" name="txt_Group_id" readonly>
                  </div>
                  <div class="form-group">
                    <label for="CampaignName">Group Name<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" title="Please enter only characters" id="txt_Group_name" name="txt_Group_name" required>
                  </div>
                </div>
                <!-- /.card-body -->
                <input type = "hidden" value = "" id = "txt_id" name = "txt_id">
                <div class="card-footer">
				  <input type= "hidden" value="edit_Group" name="editGroup" >
      ...
      <div class="modal-content scrollit">
        ...
        <div class="modal-header">
          <h4 class="modal-title">ADD A NEW USER GROUP</h4>
		   <!--<button onclick="myFunctionForFAQ();" title="Add New Campaign FAQ" style="border: none;background-color: white;"><i class="fa fa-question-circle" aria-hidden="true" style="font-size:20px;margin-top:7px;"></i></button>-->
        ...
        <div class="modal-body">
         <form action ="<?php echo $_SERVER['PHP_SELF'] ?>" method = "POST" enctype="multipart/form-data">
                <div class="card-body">
                  <div class="form-group">
                    <label for="CampaignID">Group ID<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" title="Please enter only characters" id="Group_id" name = "Group_id" placeholder="Enter Group ID" required> 
                  </div>
                  <div class="form-group">
                    <label for="CampaignName">Group Name<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" title="Please enter only characters" id="Group_name" name = "Group_name" placeholder="Enter Group Name" required>
                  </div>
                ...
                <div class="card-footer">
                  <input  type="submit" name = "addGroup" value = "Submit" class="btn btn-primary">
  ...
  <div class="modal fade" id="myModal_delete" style="opacity: 3;top : 104px !important">
    <div class="modal-dialog">
      <div class="modal-content" style="top:40px;">
        ...
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">DELETE GEOUP</h4>
         ...
         <form action = "" method = "POST">
                <div class="card-body">
                  <div class="form-group">
						...
						<input type="text" class="form-control" id="txt_deleteGroup_id" name="txt_deleteGroup_id" readonly>
					</div>
				
                </div>
				<input type = "hidden" value = "" id = "deleteGroupID" name = "deleteGroupID">
                <!-- /.card-body -->

                <div class="card-footer">
				  <input type = "hidden" value="delete_Group" name="deleteGroup" >
...
/*function myFunctionForFAQ() {
  var myWindow = window.open("help/CampaignsFAQ.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=50,width=400,height=400");
}*/
   ...
   function groupEditFun(campval)
	{
		var xhttp = new XMLHttpRequest();
			...
			var res = val.split("*");
		    //alert(res);
			document.getElementById('txt_Group_id').value=res[0];
			document.getElementById('txt_Group_name').value=res[1];
			document.getElementById('txt_id').value=res[2];
				
			//alert(res[2]);
			}
        }; 
		xhttp.open("GET", "/haloocomConnect5/pages/Agent_mongodb/ajax/get_groupedit_data.php?id="+campval, true);
		xhttp.send();	  
		// Get the modal
	...
	function groupDeleteFun(val_grpd,CampaignIDVal)
	{
		document.getElementById('deleteGroupID').value=val_grpd;
		document.getElementById('txt_deleteGroup_id').value=CampaignIDVal;
