             ...
             <table id="datatable" class="table styled-table table-bordered" cellspacing="0" bgcolor="#fff" style="border: 5px solid #36457E;width: 100%;height: 20%;"> <!--table-bordered -->
				...
				?>
					...
					if (isset($_POST["btn_submit"]) == "btn_submit") {
							...
							$sel_Call ="SELECT `phone_number`,count(`phone_number`) as countVal,`user`,`call_date` FROM `vicidial_log` where call_date >='$start_date 00:00:00' and call_date <= '$end_date 23:59:59' and user !='VDAD' GROUP BY `phone_number` HAVING COUNT(`phone_number`) >= 1";
									//echo   $sel_Call;
										//echo "<br>";
										...
										$user = $rowVal['user'];
										$call_date = $rowVal['call_date'];
										
							$sel_CallDispo ="SELECT `status` FROM `vicidial_log` where `phone_number`= '$phoneNo' and call_date >='$start_date 00:00:00' and call_date <= '$end_date 23:59:59' ORDER BY `call_date` DESC LIMIT 1;";
							$res_CallDispo = mysqli_query($db,$sel_CallDispo);
							$rowDispo =  mysqli_fetch_array($res_CallDispo);
							$Disposition = $rowDispo['status'];
						    
							
							$sel_agreementNo ="SELECT `first_name` FROM `vicidial_list` WHERE `phone_number`='$phoneNo' and first_name!='' and user !='VDAD'";
							$res_agreementNo = mysqli_query($db,$sel_agreementNo);
							$rowagreementNo =  mysqli_fetch_array($res_agreementNo);
							$agreementNo = $rowagreementNo['first_name'];
							...
							<td><?php echo $i ; ?></td>
							<td><?php echo $user ; ?></td>
							<td><?php echo $agreementNo ; ?></td>
							<td><?php echo $attempt; ?></td>
							<td><?php echo $phoneNo ; ?></td>
							<td><?php echo $call_date ; ?></td>
