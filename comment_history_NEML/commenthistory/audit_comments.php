...
function get_audited_comments($lead_id,$format,$user,$mel,$NOW_TIME,$link,$server_ip,$session_name,$one_mysql_log,$phone_number) 
	{
    global $ACcount;
    global $ACcomments;
	
	
$sel_lead = "select lead_id from vicidial_list where phone_number='$phone_number'";
//echo $sel_lead;

$rslt_lead=mysqli_query($link,$sel_lead);
$arr_lead = array();
while($row_call = mysqli_fetch_array($rslt_lead))
{
	$leadid_call = $row_call[0];
	array_push($arr_lead,$leadid_call);
}
foreach($arr_lead as $val_lead_id){
		
			$Call_leadId .= "'$val_lead_id',";
	}
$Call_leadId1 = rtrim($Call_leadId, ',');
...
if($Call_leadId1 == ''){
	...
	$leadid_val = $Call_leadId1;
}
	
    $stmt="SELECT user_id,comment,timestamp from vicidial_comments where lead_id IN ($leadid_val) order by timestamp desc;";
	//echo "===>".$stmt;
    $rslt=mysql_to_mysqli($stmt, $link);
    if ($mel > 0) 
