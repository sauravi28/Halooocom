...
///BREAKS//
// ############ total Lunch Break ############################
$sel_lb = "select sum(pause_sec) from vicidial_agent_log where event_time >='$query_date_BEGIN' and event_time <= '$query_date_END' and user LIKE '$SuserName%' and sub_status = 'LB'";
$res_lb = mysqli_query($link,$sel_lb);
$row_lb = mysqli_fetch_array($res_lb);
$lbk_sec = $row_lb[0];
$totlb_sec = $totlb_sec + $lbk_sec;
$lbk_time = gmdate("H:i:s", $lbk_sec);
// ############ total Lunch Break ############################

// ############ total MD Break ############################
$sel_md = "select sum(pause_sec) from vicidial_agent_log where event_time >='$query_date_BEGIN' and event_time <= '$query_date_END' and user LIKE '$SuserName%' and sub_status = 'MD'";
$res_md = mysqli_query($link,$sel_md);
$row_md = mysqli_fetch_array($res_md);
$mdbk_sec = $row_md[0];
$totmd_sec = $totmd_sec + $mdbk_sec;
$mdbk_time = gmdate("H:i:s", $mdbk_sec);
// ############ total MD Break ############################

// ############ total TEA Break ############################
$sel_TEA = "select sum(pause_sec) from vicidial_agent_log where event_time >='$query_date_BEGIN' and event_time <= '$query_date_END' and user LIKE '$SuserName%' and sub_status = 'TEA'";
$res_TEA = mysqli_query($link,$sel_TEA);
$row_TEA = mysqli_fetch_array($res_TEA);
$TEAbk_sec = $row_TEA[0];
$totTEA_sec = $totTEA_sec + $TEAbk_sec;
$TEAbk_time = gmdate("H:i:s", $TEAbk_sec);
// ############ total TEA Break ############################

// ############ total BIO Break ############################
$sel_BIO = "select sum(pause_sec) from vicidial_agent_log where event_time >='$query_date_BEGIN' and event_time <= '$query_date_END' and user LIKE '$SuserName%' and sub_status = 'BIO'";
$res_BIO = mysqli_query($link,$sel_BIO);
$row_BIO = mysqli_fetch_array($res_BIO);
$BIObk_sec = $row_BIO[0];
$totBIO_sec = $totBIO_sec + $BIObk_sec;
$BIObk_time = gmdate("H:i:s", $BIObk_sec);
// ############ total BIO Break ############################
	...
	if ($show_percentages) {
		...
		$Toutput = "<tr><td> $Sfull_name </td><td> <a href=\"./user_stats.php?user=$RAWuser\">$Suser</a> </td><td> $Suser_group </td><td> $Slast_user_group$BBD_filler </td><td>$logindt</td><td> $loginTime </td><td>$logoutdt</td><td> $logoutTime </td><td> $Scalls </td><td> $pfUSERtime_MS </td><td> $pfUSERtotPAUSE_MS </td><td> $pfUSERavgPAUSE_MS </td><td> $pfUSERtotWAIT_MS </td><td> $pfUSERavgWAIT_MS </td><td> $pfUSERtotTALK_MS </td><td> $pfUSERavgTALK_MS </td><td> $pfUSERtotDISPO_MS </td><td> $pfUSERavgDISPO_MS </td><td> $pfUSERtotDEAD_MS </td><td> $pfUSERavgDEAD_MS </td><td> $pfUSERtotCUSTOMER_MS </td><td> $pfUSERavgCUSTOMER_MS </td><td>$BIObk_time</td><td>$lbk_time</td><td>$mdbk_time</td><td>$TEAbk_time</td><td>$SstatusesHTML</td></tr>";
	}
	...
	if ($show_percentages) {
		...
		$CSV_lines.=preg_replace('/\s/', '', "\"$SuserRAW\",\"$Suser_group\",\"$Slast_user_group$BBD_csv_filler\",\"$logindt\",\"$loginTime\",\"$logoutdt\",\"$logoutTime\",\"$Scalls\",\"$pfUSERtime_MS\",\"$pfUSERtotPAUSE_MS\",\"$pfUSERavgPAUSE_MS\",\"$pfUSERtotWAIT_MS\",\"$pfUSERavgWAIT_MS\",\"$pfUSERtotTALK_MS\",\"$pfUSERavgTALK_MS\",\"$pfUSERtotDISPO_MS\",\"$pfUSERavgDISPO_MS\",\"$pfUSERtotDEAD_MS\",\"$pfUSERavgDEAD_MS\",\"$pfUSERtotCUSTOMER_MS\",\"$pfUSERavgCUSTOMER_MS\",\"$BIObk_time\",\"$lbk_time\",\"$mdbk_time\",\"$TEAbk_time\"$CSVstatuses");
	}
	$CSV_lines.="\n";
...
$totlb_sectime = gmdate("H:i:s", $totlb_sec);
$totmd_sectime = gmdate("H:i:s", $totmd_sec);
$totTEA_sectime = gmdate("H:i:s", $totTEA_sec);
$totBIO_sectime = gmdate("H:i:s", $totBIO_sec);
...
if ($show_percentages) {
	...
	$ASCII_text.="<tr><td colspan='8'>  "._QXZ("TOTALS",33)." "._QXZ("AGENTS",32,"r").":$TOT_AGENTS$BBD_filler </td><td> $TOTcalls</td><td>$TOTtime_MS</td><td>$TOTtotPAUSE_MS</td><td> $TOTavgPAUSE_MS </td><td>$TOTtotWAIT_MS</td><td> $TOTavgWAIT_MS </td><td>$TOTtotTALK_MS</td><td> $TOTavgTALK_MS </td><td>$TOTtotDISPO_MS</td><td> $TOTavgDISPO_MS </td><td>$TOTtotDEAD_MS</td><td> $TOTavgDEAD_MS </td><td>$TOTtotCUSTOMER_MS</td><td> $TOTavgCUSTOMER_MS </td><td>$totBIO_sectime</td><td>$totlb_sectime</td><td>$totmd_sectime</td><td>$totTEA_sectime</td><td>$SUMstatusesHTML</td></tr>";
	//$ASCII_text.="+-----------------+----------+----------------------+----------------------+".$BBD_header."--------+-----------+----------+--------+----------+--------+----------+--------+----------+--------+----------+--------+----------+--------+$statusesHEAD\n";
...
if ($show_percentages) {
	...
	$CSV_total=preg_replace('/\s/', '', "\"\",\"\",\""._QXZ("TOTALS")."\",\""._QXZ("AGENTS").":$TOT_AGENTS\",\"\",\"\",\"\",\"\",\"$TOTcalls\",\"$TOTtime_MS\",\"$TOTtotPAUSE_MS\",\"$TOTavgPAUSE_MS\",\"$TOTtotWAIT_MS\",\"$TOTavgWAIT_MS\",\"$TOTtotTALK_MS\",\"$TOTavgTALK_MS\",\"$TOTtotDISPO_MS\",\"$TOTavgDISPO_MS\",\"$TOTtotDEAD_MS\",\"$TOTavgDEAD_MS\",\"$TOTtotCUSTOMER_MS\",\"$TOTavgCUSTOMER_MS\",\"$totBIO_sectime\",\"$totlb_sectime\",\"$totmd_sectime\",\"$totTEA_sectime\"$CSVSUMstatuses\n\n");
