...
{
        if(shell_exec("sshpass -p 'Fusion@Call%Rec%Data#369' scp -r /srv/www/htdocs/Haloo_Voicelogs/sftp_folder/$current_year solvent1@192.168.1.5:D:/Call_Recordings/"))
	{
...
{
        if(shell_exec("sshpass -p 'Fusion@Call%Rec%Data#369' scp -r /srv/www/htdocs/Haloo_Voicelogs/sftp_folder/$current_year/$current_month solvent1@192.168.1.5:/$current_year/"))
        {
...
{
        if(shell_exec("sshpass -p 'Fusion@Call%Rec%Data#369' scp -r /srv/www/htdocs/Haloo_Voicelogs/sftp_folder/$current_year/$current_month/$current_day solvent1@192.168.1.5:/$current_year/$current_month/"))
        {
...
sleep(2);
 ...
 //foreach($myfiles as $datefolder){
	 ...
	 $mydir1 = '../voicefiles_Wav/'.$datefolder."/";
		 //echo $mydir1."<br>"; 
			...
			if(substr($voiceFiles,-4) == ".wav"){
				...
				if($row_voiceFile[0] > 0){
					...
					//Dublicate files checking.. for avoiding 
						...
						//$filecopy =shell_exec(" sshpass -p 'password'  scp -r /srv/www/htdocs/Haloo_Voicelogs/voicefiles/$voiceFilesPath solvent1@192.168.1.5:D:/Recordings/2024/03/$voiceFiles");
						$filecopy = shell_exec("sshpass -p 'Fusion@Call%Rec%Data#369'  scp -r /srv/www/htdocs/voicefiles_Wav/$voiceFilesPath solvent1@192.168.1.5:/$current_year/$current_month/$current_day/$voiceFiles");
