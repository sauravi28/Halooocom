		...
		if ($affected_rows > 0)
			...
			if ($qc_features_active > 0)
				...
				if ($audit_comments_active)
					{
					get_audited_comments($lead_id,$format,$user,$mel,$NOW_TIME,$link,$server_ip,$session_name,$one_mysql_log,$phone_number);
					}
...
if ($ACTION == 'CalLBacKLisT')
	...
	while ($callbacks_count>$loop_count)
		...
		if ($callback_list_calltime == 'ENABLED')
			...
			$callback_time12hr = date('Y-m-d h:i:s', strtotime($callback_time[$loop_count]));
			$entry_time12hr = date('Y-m-d h:i:s', strtotime($entry_time[$loop_count]));
			
		$CBoutput = "$row[0]-!T-$row[1]-!T-$row[2]-!T-$callback_id[$loop_count]-!T-$lead_id[$loop_count]-!T-$campaign_id[$loop_count]-!T-"._QXZ("$status[$loop_count]")."-!T-$entry_time12hr-!T-$callback_time12hr-!T-$comments[$loop_count]-!T-$PHONEdialable-!T-$alt_phone";
		echo "$CBoutput\n";
		$loop_count++;
		}
