<?php
	...
	$date = date("Y-m-d");
	$sql_ticket = "select cc.client_email_id_c, cc.email_update_c,c.id,c.name,c.case_number,c.date_entered,c.date_modified,c.modified_user_id,c.state from cases c left join cases_cstm cc on c.id = cc.id_c where c.date_modified like '$date%' and c.state in ('Closed','Resolved') and cc.email_closeupdate_c='0'";		 //assigned_user_id //order by date_modified DESC limit 1
	//echo $sql_ticket;
	//die;
	...
	while($row_ticket = mysqli_fetch_array($result_ticket)) {
		...
		require_once "/var/www/html/Mail/mime.php"; // PEAR Mail_Mime packge
				...
				$to = $client_email_id;//"<sauravi.sarode@haloocom.com>";
				$subject = "Haloocom Support Ticket - ".$case_number ." Closed."; //'Re:' . $Lastemailsubject; 
				...
				$mail = $smtp->send($to, $headers, $body);
					if (PEAR::isError($mail))
					...
					else
						{
							...
							$update_emailUpdate = 'update cases_cstm set email_closeupdate_c="1"  where id_c="'.$ticketId.'"'; 
