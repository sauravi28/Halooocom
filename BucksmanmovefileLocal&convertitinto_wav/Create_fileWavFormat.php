	 ...
	 $mydir1 = '/srv/www/htdocs/Haloo_Voicelogs/voicefiles/'.$datefolder."/";
		...
		foreach($myfiles1 as $voiceFiles){
			...
			if(substr($voiceFiles,-4) == ".gsm"){
				...
				$file_split = explode("-",$voiceFiles);
				$first_part = $file_split[0];
				$second_part = $file_split[1];
				$third_part = $file_split[2];
				$new_filename = $first_part.$second_part."-".$third_part;
		
				
				$voicefilePH = explode("_",$new_filename);
				$voicefilesource = $voicefilePH[1];

					$stmt_val = "select last_name,field5,postal_code from vicidial_list where phone_number='$voicefilesource'";
