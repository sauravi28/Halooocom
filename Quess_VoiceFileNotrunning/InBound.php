  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card card-default">
          <div class="card-header">
            <h3 class="card-title">INBOUND REPORT</h3>
            <br><br>
			  <?php
						   ...
						   if (isset($_POST["btn_submit"]) == "btn_submit") {
							 ...
							 if ($start_date !="" && $end_date !="") {	
									...
									$mongo->executeBulkWrite('candidateinfo.repostStatus', $bulk);
											...
											foreach ($_POST['D4_User'] as $selected_D4Option) {
												...
												foreach($rows_talkTIme as $rows){
													...
													if(strlen($rows->customerno) > 10){
														if(strlen($rows->customerno) == 12){
															$phoneNumber = substr($rows->customerno, 2);
														}elseif(strlen($rows->customerno) == 11){
															$phoneNumber = substr($rows->customerno, 1);
														}
												...
												$callStatus = $rows->disposition;
												if($callStatus == "Busy - Busy"){
														$callStatus = "Busy";
													}
													$dialstatus = $rows->dialstatus;
													if(($dialstatus == 'DROP') || ($dialstatus == 'BUSY') || ($dialstatus == 'CANCEL')){
														$dialstatus ="No_Answere";
													}else{
														$dialstatus ="Answered";
													}
												...
												$mongo = new MongoDB\Driver\Manager("mongodb://localhost:27017");    
												$bulk = new MongoDB\Driver\BulkWrite;
												$filter = ["phone_number" => "$phoneNumber","archival" => 0];							
												//echo "==>".print_r($filter); 
												$options = []; 
												$qry_userId = new MongoDB\Driver\Query($filter,$options);											
												$rows_phone_number = $mongo->executeQuery("candidateinfo.candidate_Information", $qry_userId); 
												$arr_phone_number = $rows_phone_number->toArray();
												$array_phone_number = json_decode(json_encode($arr_phone_number), true);							        
												$numberStatus = $array_phone_number[0]['phone_number'];
													if($numberStatus == ''){
														$numberStatus = "NEW";
													}else{
														$numberStatus = "EXIST"; 
											...
											<!--  <td class="col-xs-2" style="text-align: center;"><?php //echo $rows->call_type; ?></td> -->
											  <td class="col-xs-2" style="text-align: center;"><?php echo $ExtensionData; ?></td>
											  <td class="col-xs-2" style="text-align: center;"><?php echo $phoneNumber; ?></td>
											   <td class="col-xs-1" style="text-align: center;"><?php echo $numberStatus; ?></td>
											    <td class="col-xs-1" style="text-align: center;"><?php if($dialstatus=="No_Answere"){echo "No Answer";}else if($dialstatus=="Answered"){ echo "Answer";}?></td>
											  <!--<td class="col-xs-1" style="text-align: center;"><?php //echo $callStatus; ?></td>-->
											  
											  <td class="col-xs-1" style="text-align: center;"><?php echo $UserLoaction; ?></td>
											  <td class="col-xs-1" style="text-align: center;"><?php if($TotalTalkTime==""){echo "00:00:00";}else{ echo $TotalTalkTime;} ?></td>
												 ...
												 <?php if($rows->voice != ''){
																...
																if (strpos($myString, $word) !== false) {
																	...
																	$valFile = "Word Not Found!";
																}
																if($valFile == "Word Found!"){

																	$strreplaceVoicefile = str_replace("banwfo","bglwfo",$myString);
																
													             }
																 else{
																	 $strreplaceVoicefile = $rows->voice;
																 }
													 ?>
												<td class="col-xs-1" style="text-align: center;"><a href="<?php echo "../../../voicefiles/".$strreplaceVoicefile; ?>">Download</a></td>		
											  <?php }else{?>	
											  <td class="col-xs-1" style="text-align: center;"><a href="#"></a></td>		
											  <?php }?>	
											 </tr>							
