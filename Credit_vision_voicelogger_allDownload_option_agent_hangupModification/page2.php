...
?>
	...
	<b><font size="3">Select Date </font> </b> &nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp; 
	 ...
	 <b><font size="3">Select User Group</font><i class="fa fa-asterisk" style="color:red"></i> </b>&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;<select id="txt_user_group" name = "txt_user_group" onChange="getgroup(this.value);" style="height: 28px;" required>
								<option value ="<?php echo $txt_user_group; ?>" selected><?php echo $txt_user_group; ?></option>
								<?php 
								...
								$sql1 = "select admin_viewable_groups from vicidial_user_groups where user_group = '$row[0]'";
								//echo "==> UserGroup ==>".$sql1;
								//echo "<br>";
								$res1 = mysqli_query($db,$sql1);
								$row1 = mysqli_fetch_array($res1);
								$admin_viewable_groups = trim($row1[0]);
								//echo $admin_viewable_groups;
								
								if($admin_viewable_groups == "---ALL---")
				                {
									$query = "SELECT user_group FROM vicidial_user_groups";
								    //echo "======>".$query;
								...
								$arr = explode(" ",$admin_viewable_groups);
									//print_r($arr);
								$str = implode("','",$arr);
								
								$selectgrp = "SELECT user_group FROM vicidial_user_groups WHERE user_group in ('$str')";
									$result1 = mysqli_query($db,$selectgrp);
									while ($row1 = mysqli_fetch_array($result1)){
										echo "<option value = ".$row1['user_group'].">" . $row1['user_group'] . "</option>";
									}	
								}	
?>
	 ...
	 <b><font size="3">Select Campaign</font><i class="fa fa-asterisk" style="color:red"></i> </b>&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;<select id="txt_campaign" name = "txt_campaign[]" MULTIPLE  size = 8 required>
	  ...
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <?php if($download == 'ok'){?><a href='<?php echo "backup_$user.tgz"?> '  target="_self"onclick="test()">Click to Download</a> <?php }elseif($download == 'fail'){ echo "Data not Available"; } ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
function getgroup(val) {
	var dateVal = document.getElementById("daterange").value;
	//alert(val);
	$.ajax({
	type: "POST",
	url: "get_campaignName.php",
	data:'user_groupID='+val+"&dateVal="+dateVal,
	success: function(data){
		alert(data);
		$("#txt_campaign").html(data);
		
	}
	});
}

$('#txt_campaign').change(function() {
        if($(this).val() == 'all')
        {
            $('#txt_campaign option').prop('selected', true);
        }
    });
