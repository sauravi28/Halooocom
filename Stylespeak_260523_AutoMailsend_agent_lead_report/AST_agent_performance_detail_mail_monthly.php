...
//step1
//file creation to export to excel  haloo
$date_yest = date('Y-m-d',strtotime("-1 days"));
$file_yest = "Agent_per_".$date_yest.".csv";
$date = date('Y-m-d');
$file1 = "agent_performance_detail_monthly_Report_".$date.".csv";

unlink($file_yest);
unlink($file1);

$file_toExport = $file1;
$handle = fopen($file_toExport, 'a') or die('Cannot open file:  '.$file_toExport);
fwrite($handle,'');

$PHP_AUTH_USER="6666";
$PHP_AUTH_PW="haloocom2023";
...
// ############ logout time ##########
	...
	if ($show_percentages) {
		...
		$Toutput = "<tr><td> $Sfull_name </td><td> <a href=\"./user_stats.php?user=$RAWuser\">$Suser</a> </td><td> $Suser_group </td><td> $Slast_user_group$BBD_filler </td><td>$logindt</td><td> $loginTime </td><td>$logoutdt</td><td> $logoutTime </td><td> $Scalls </td><td> $pfUSERtime_MS </td><td> $pfUSERtotPAUSE_MS </td><td> $pfUSERavgPAUSE_MS </td><td> $pfUSERtotWAIT_MS </td><td> $pfUSERavgWAIT_MS </td><td> $pfUSERtotTALK_MS </td><td> $pfUSERavgTALK_MS </td><td> $pfUSERtotDISPO_MS </td><td> $pfUSERavgDISPO_MS </td><td> $pfUSERtotDEAD_MS </td><td> $pfUSERavgDEAD_MS </td><td> $pfUSERtotCUSTOMER_MS </td><td> $pfUSERavgCUSTOMER_MS </td><td>$SstatusesHTML</td></tr>";
	}
...
fwrite($handle, $CSV_lines);
fwrite($handle, $CSV_total);
