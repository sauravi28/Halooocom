...
date_default_timezone_set('Asia/Kolkata');
	...
	$select_stmt = "SELECT recording_filename,callDate FROM `salesforceApi_data` WHERE  api_update='0' and  callDate like '$currant_date%'";
					...
					while($row1 = mysqli_fetch_array($resultstmt)){
						...
						$recording_filename1ext = str_replace("wav","gsm",$recording_filename1);
								
						
						$sel_id = "select count(filename),filename from voicefiles where date like '$CalldatVal%' and filename='$recording_filename1ext'";
						//echo $sel_id;
						//die;
						...
						$cnt =$row_id[0];
						$filename =$row_id[1];
						$filename_ext = str_replace("gsm","wav",$filename);
						
						if($cnt > 0 )
						{
								$sel_apiVal = "select recording_filename,callDate,lead_id,username,loanId,disposition,sub_disposition1,sub_disposition2,comments,amount,popupdate,attempt_type,attempt_status,mode_of_payment,session_id,phone_number,id from salesforceApi_data where recording_filename = '$filename_ext'";
								//echo $sel_apiVal;die;
							...
							curl_setopt_array($curl, array(
							  ...
							  CURLOPT_HTTPHEADER => array(
								'agency_name:sr9',
								'bot-api-client-id:t3rx0UoPeq',
								'bot-api-client-key:PWA465DMmK',
								'Content-Length:0'
								),
							));
