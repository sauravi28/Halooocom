 ...
 function get_all_directory_and_files($dir){
     ...
     foreach ($dh as $item) {
		...
		echo "File==".$file;
			    ...
			    $campaign=$time_file['4'];
				$group_name=$time_file['3'];
            ...
            if($cnt == 0)
			{
				...
				$destinationFilePath = "/srv/www/htdocs/Haloo_Voicelogs/voicefiles_campaign/".$date_complete."/".$campaign."/".$file;
			
				if( !copy($filePath, $destinationFilePath) ) {  
					echo "File can't be moved!".$file;  
				}  
				else {  
			...
			$link = mysqli_connect("localhost","root","Hal0o(0m@72427242","asterisk");
				 ...
				 // Dispo//
						...
						if($row_fileid[0] !=''){
							...
							$sel_status = "select count(*),field36,alt_dial From vicidial_log where uniqueid = '$row_fileid[0]'";
							echo $sel_status;
							//echo "<br>";
							...
							//Incoming//
							$sel_status_in = "select count(*),field36 From vicidial_closer_log  where closecallid = '$row_fileid[0]'";
							echo $sel_status_in;
							//echo "<br>";
							...
							$alt_dial = $row_status[2]; //NONE,MAIN,MANUAL
							if($alt_dial == 'NONE' || $alt_dial == 'MAIN'){
								$call_mode = "AUTO";
							}
							else{
								$call_mode = "MANUAL";
           	    ...
           	    $sql = "insert into voicefiles(date,time,source,destination,duration,filename,campaign,disposition,call_type,call_mode,group_name)values('$date_cal','$time','$source','$destination','$duration','$filename1','$campaign','$Disposition','$call_type','$call_mode','$group_name')";
